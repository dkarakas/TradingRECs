...
char infoUpdate[MAX_REQUEST] = "";
char REC[MAX_REQUEST] = "";
char prevTx[MAX_REQUEST] = "";
char owner[MAX_REQUEST] = "";
byte * requests[2] = { infoUpdate, REC };
ecc_key key;
...
WebTransfer webTransfers[] = {
	{.url = "https://192.168.43.242/metadataUpdate" },
	{.url = "https://192.168.43.242/newREC"} };
...
static CURL *CurlSetupEasyHandle(char *url, char * req, HttpResponse *response)
{
    ...
    long allowedProtocols = CURLPROTO_HTTPS; //CURLPROTO_HTTP | CURLPROTO_HTTPS;
	...
	if ((res = curl_easy_setopt(easyHandle, CURLOPT_SSL_CTX_FUNCTION, MyCallback)) != CURLE_OK) {
		LogCurlError("curl_easy_setopt CURLOPT_SSL_CTX_FUNCTION", res);
		goto errorLabel;
	}

	char *path = Storage_GetAbsolutePathInImagePackage("certs/ca-crt.pem");
	if ((res = curl_easy_setopt(easyHandle, CURLOPT_CAINFO, path)) != CURLE_OK) {
		LogCurlError("curl_easy_setopt CURLOPT_CAINFO", res);
		goto errorLabel;
	}

	if ((res = curl_easy_setopt(easyHandle, CURLOPT_POSTFIELDS, req)) != CURLE_OK) {
		LogCurlError("curl_easy_setopt CURLOPT_POSTFIELDS", res);
		goto errorLabel;
	}

    // Turn on verbosity of cURL.
...
static void CurlTimerEventHandler(EventData *eventData)
{
	...
	//Log_Debug("CurlTimerEventHandler")
        ...
        if ((curlMessage != NULL) && (curlMessage->msg == CURLMSG_DONE)) {
            ...
            for (size_t i = 0; i < sizeof(webTransfers) / sizeof(*webTransfers); i++) {
                if (webTransfers[i].easyHandle == e) {
					...
					//Response from smart meter generator information update
					if (i == 0) {
						char * regexString = ".*\\{\"prevTx\":\"(.+)\",\"owner\":\"(.+)\",\"status\".*";
						size_t maxGroups = 3;

						regex_t regexCompiled;
						regmatch_t groupArray[maxGroups];
						int ret = regcomp(&regexCompiled, regexString, REG_EXTENDED);
						if (ret)
						{
                        ...
                        ret = regexec(&regexCompiled, webTransfers[i].httpResponse.content.data, maxGroups, groupArray, 0) ;
						if (ret == 0)
						{
							unsigned int g = 0;
							for (g = 0; g < maxGroups; g++)
							{
								char updateResponse[strlen(webTransfers[i].httpResponse.content.data) + 1];
								strcpy(updateResponse, webTransfers[i].httpResponse.content.data);
                                updateResponse[groupArray[g].rm_eo] = 0;
								if(g == 1 && groupArray[g].rm_eo - groupArray[g].rm_so < MAX_REQUEST)
									strcpy(prevTx, updateResponse + groupArray[g].rm_so);
								if(g == 2 && groupArray[g].rm_eo - groupArray[g].rm_so < MAX_REQUEST)
									strcpy(owner, updateResponse + groupArray[g].rm_so);
							}
						}
...
static int CurlInit(void)
{
    ...
    for (size_t i = 0; i < sizeof(webTransfers) / sizeof(*webTransfers); i++) {//Initially the REC request is set to empty string
        webTransfers[i].easyHandle =
            CurlSetupEasyHandle(webTransfers[i].url, requests[i], &webTransfers[i].httpResponse);
...
int WebClient_StartTransfer(int idTransfer)
{
	int res = 0;
	if (curlTransferInProgress == 0) {
		struct timespec currentTime;
		clock_gettime(CLOCK_MONOTONIC, &currentTime);
		CURLMcode res;
		if (idTransfer >= sizeof(webTransfers) / sizeof(*webTransfers)) {
			return -1;
		}

		if ((res = curl_multi_remove_handle(curlMulti, webTransfers[idTransfer].easyHandle)) !=
			CURLM_OK) {
			LogCurlError("curl_multi_remove_handle", res);
			return -1;
        }

        if(idTransfer == 0){//Handling information update
            createUpdateInfo(requests[idTransfer], &key);
            if(requests[idTransfer] == NULL){
                Log_Debug("Could not create update info request");
                return -1;
            }
            if(webTransfers[idTransfer].easyHandle != NULL){
                int res = curl_easy_setopt(webTransfers[idTransfer].easyHandle, CURLOPT_POSTFIELDS, requests[idTransfer]);
                if (res != CURLE_OK) {
                    LogCurlError("curl_easy_setopt CURLOPT_POSTFIELDS", res);
                }
            }else{
                Log_Debug("Need to initialize easy handles first!");
            }
        }

        if(idTransfer == 1){//Handling smart meter information update
            if(strlen(prevTx) == 0 || strlen(prevTx) == 0){
                Log_Debug("There has not been a successful smart meter information update!");
                return -1;
            }
            createREC(requests[idTransfer], prevTx, owner, &key);
            if(requests[idTransfer] == NULL){
                Log_Debug("Could not create REC request");
                return -1;
            }
            if(webTransfers[idTransfer].easyHandle != NULL){
                int res = curl_easy_setopt(webTransfers[idTransfer].easyHandle, CURLOPT_POSTFIELDS, requests[idTransfer]);
        ...
        if ((res = curl_multi_add_handle(curlMulti, webTransfers[idTransfer].easyHandle)) != CURLM_OK) {
			LogCurlError("curl_multi_add_handle", res);
			return -1;
		}

		webTransfers[idTransfer].startTime = currentTime;
		curlTransferInProgress++;
	}
	return res;
}
