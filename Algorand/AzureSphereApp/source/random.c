...
#include <sys/random.h>//azure sphere HRNG
...
int wc_GenerateSeed(OS_Seed* os, byte* output, word32 sz)
{
    ...
    */
    ...
    //Using Azure Sphere Random number generator 
    int wc_GenerateSeed(OS_Seed* os, byte* output, word32 sz)
    {
        word32 i;
        if(getrandom(output, sz, 0x0000) != sz){
            Log_Debug("Hardware number generator failed!");
        }
