...
#include "include/applibs_versions.h"
#include <applibs/log.h>
#include <applibs/storage.h>
#include <tlsutils/deviceauth_curl.h>

#include "include/epoll_timerfd_utilities.h"
#include "include/log_utils.h"
#include "include/web_client.h"
#include "include/regex.h"

#include "include/req_construct.h"

char reqReg[1024] = "";
char reqCert[1024] = "";
char prevTx[1024] = "";
char owner[1024] = "";
byte * requests[2] = { reqReg, reqCert };
...
WebTransfer webTransfers[] = {
	...
	//{.url = "https://httpstat.us/400?sleep=1"}};
	//{.url = "https://mutualauthtest.azurewebsites.net/api/authTestFunc?code=rHwqwLLcpxF14mpimQAT3xzYsIOGUmQ7eC6klN4Yu6anyP0DlXaFUg==&name=Dimcho"} };
	{.url = "https://192.168.43.242/register" },
	{.url = "https://192.168.43.242/newCert"} };
        ...
        if ((curlMessage != NULL) && (curlMessage->msg == CURLMSG_DONE)) {
            ...
            for (size_t i = 0; i < sizeof(webTransfers) / sizeof(*webTransfers); i++) {
                if (webTransfers[i].easyHandle == e) {
					...
					//That means the user has registered
					if (i == 0) {
						...
						int ret;
						if (ret = regcomp(&regexCompiled, regexString, REG_EXTENDED))
						{
							printf("Could not compile regular expression. %d \n",ret);
							return;
						};

						if (regexec(&regexCompiled, webTransfers[i].httpResponse.content.data, maxGroups, groupArray, 0) == 0)
						{
							unsigned int g = 0;
							for (g = 0; g < maxGroups; g++)
							{
								...
								char sourceCopy[strlen(webTransfers[i].httpResponse.content.data) + 1];
								strcpy(sourceCopy, webTransfers[i].httpResponse.content.data);
								sourceCopy[groupArray[g].rm_eo] = 0;
								printf("Group %u: [%2u-%2u]: %s\n", g, groupArray[g].rm_so, groupArray[g].rm_eo, sourceCopy + groupArray[g].rm_so);
								if(g == 1)
									strcpy(prevTx, sourceCopy + groupArray[g].rm_so);
								if(g == 2)
									strcpy(owner, sourceCopy + groupArray[g].rm_so);
							}
							Log_Debug(prevTx);
							Log_Debug("\n");
							Log_Debug(owner);
							Log_Debug("\n");
							certRequest(requests[1], prevTx, owner);
							int res;
							if (res = (curl_easy_setopt(webTransfers[i+1].easyHandle, CURLOPT_POSTFIELDS, requests[1])) != CURLE_OK) {
								LogCurlError("curl_easy_setopt CURLOPT_POSTFIELDS", res);
							}
						}
...
static int CurlInit(void)
{
	...
	regRequest(requests[0]);
	//Registrations should return owner address and txId of registration.
	certRequest(requests[1], prevTx, owner);
    for (size_t i = 0; i < sizeof(webTransfers) / sizeof(*webTransfers); i++) {
