...
async function main(argv) {
    if(argv.length !== 4 || !(argv[2] === '--newNetwork') || !(argv[3] === 'true' || argv[3] === 'false')) {
        console.log('Usage: node client.js --newNetwork');
        console.log('--newNetwork for new network set the value to true, otherwise false');
        process.exit(1);
    }
    if(argv[3] === 'true' && fs.existsSync('./wallet')){
...
clientMqtt.on('message', function (msg) {
    ...
    invokeChaincode('newCoin',JSON.stringify(JSON.parse(msg.data)));
});
...
invokeChaincode('createUser',JSON.stringify(prodUnit));
// //
// const coinListing = {
...
//     minPrice: 12
// }
// invokeChaincode('createCoinListing',JSON.stringify(coinListing));
