...
class CryptoSuite_ECDSA_AES extends api.CryptoSuite {
	...
	importAzureKey(key){
		var theKey = "";
		try {
			key = KEYUTIL.getKey({xy: key, curve: this._curveName}, null, "pkcs8pub");
		} catch (err) {
			console.log('Failed to parse key from Azure: ' + err);
			error = new Error('Failed to parse key from Azure: ' + err);
		}
		if (key && key.type && key.type === 'EC') {
			theKey = new ECDSAKey(key);
			logger.debug('importKey - have the key %j', theKey);
		} else {
			error = new Error('Does not understand the content');
		}
		return theKey;
	}
	/**
	...
	verifyAzure(key, signature, digest) {
		if (typeof key === 'undefined' || key === null) {
	...
	checkMalleability(signature, curveParams){
		return _checkMalleability(signature, curveParams);
	}
	...
	verify(key, signature, digest) {
		...
		// console.log("Before _checkMalleability")
		if (!_checkMalleability(signature, key._key.ecparams)) {
			logger.error(new Error('Invalid S value in signature. Must be smaller than half of the order.').stack);
			return false;
		}
		// console.log("After _checkMalleability")
		// console.log(key.getPublicKey()._key.pubKeyHex)
		const pubKey = this._ecdsa.keyFromPublic(key.getPublicKey()._key.pubKeyHex, 'hex');
		// console.log("After keyFromPublic")
