...
// Included from High Level app which communicates with real time core
#include <string.h>
#include <ctype.h>
#include <stdbool.h>
#include <unistd.h>
...
//CODE FROM HIGH LEVEL APP FOR COMMUNICATION WITH REALTIME CORE
...
static int allowSendReadings = 0;
...
static void SendMessageToIoTHub(void)
{
    if (connectedToIoTHub) {
        // Send a message
        AzureIoT_SendMessage("{\"smartMeter\":\"SM1\",\"meterreading\":\"100000\",\"timestamp\":\"Time\"\}");
...
/// <summary>
///     Handle socket event by reading incoming data from real-time capable application and send IoT Hub Message.
/// </summary>
static void SocketEventHandler(EventData *eventData)
{
	...
	//ALSO SEND THE READING TO IOT HUB
	if (connectedToIoTHub) {
		char buff[512] = "{\"smartMeter\":\"SM1\",\"meterreading\":\"";
		char relevantInfoRxBuff[6];
		memcpy(relevantInfoRxBuff, &rxBuf, 5);
		relevantInfoRxBuff[5] = '\0';
		strcat(buff, relevantInfoRxBuff);
		strcat(buff, "\",\"timestamp\":\"Time\"\}");

		// Send a message
		//AzureIoT_SendMessage("{\"smartMeter\":\"SM1\",\"meterreading\":\"100000\",\"timestamp\":\"Time\"\}");
		if (allowSendReadings == 1) {
			AzureIoT_SendMessage(buff);
		}
...
static void ButtonPollTimerHandler(EventData *eventData)
{
    ...
    if (IsButtonPressed(sendMessageButtonGpioFd, &messageButtonState)) {
		allowSendReadings = !allowSendReadings;
    }
}
...
{
    Log_Debug("INFO: Closing GPIOs, Reat Time Core FDs and Azure IoT client.\n");
